...
const toUiProduct = (p: ApiProduct): UiProduct => {
  ...
  return {
    ...
    priceSale: anyP.priceSale ?? anyP.salePrice ?? anyP.discountPrice ?? anyP.prices?.sale ?? undefined,
...
export default function HomeScreen({ navigation }: any) {
  ...
  const Header = (
    <View style={{ paddingTop: insets.top + 4 }}>
      ...
      {/* Accessories section intentionally removed */}
          ...
          <TouchableOpacity onPress={() => navigation.navigate('Product', { id: item.id })}>
            <ProductCard
              item={{
                id: item.id,
                title: item.title,
                priceMrp: item.priceMrp ?? item.priceSale ?? 0,
                priceSale: item.priceSale ?? item.priceMrp ?? 0,
                images: item.images,
              }}
              onPress={() => navigation.navigate('Product', { id: item.id })}
              // If your ProductCard supports styling props, you can add:
              // accentColor={accent}
            />
          </TouchableOpacity>
        )}
